...
#if defined(LLAB_BITMAP_V4)
using llab::bitmap_ladder_order_book::Level;
using llab::bitmap_ladder_order_book::OrderBook;
#elif defined(LLAB_BITMAP_V5)
using llab::bitmap_backshift_order_book::Level;
using llab::bitmap_backshift_order_book::OrderBook;
#elif defined(LLAB_BITMAP_V6)
using llab::bitmap_packed_order_book::Level;
using llab::bitmap_packed_order_book::OrderBook;
#else
using llab::dense_ladder_order_book::Level;
...
int main(int argc, char **argv) {
    ...
    while (std::getline(input, line)) {
        ...
        if (line.starts_with("EXPECT ")) {
            ...
            require(book.best_bid() == std::optional{Level{bid_price, bid_quantity}},
                    "best bid mismatch");
            require(book.best_ask() == std::optional{Level{ask_price, ask_quantity}},
                    "best ask mismatch");
