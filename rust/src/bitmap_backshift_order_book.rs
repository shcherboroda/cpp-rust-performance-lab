...
impl Default for Order {
    fn default() -> Self {
        Self { side: Side::Bid, price: 0, quantity: 0 }
    }
}
...
impl OrderBook {
    ...
    fn add(&mut self, id: OrderId, side: Side, p: Price, q: Quantity) -> Result<(), &'static str> {
        ...
        if !self.orders.insert(id, Order { side, price: p, quantity: q }) {
            return Err("index");
        }
    ...
    fn remove(&mut self, id: OrderId) -> Result<(), &'static str> {
        ...
        self.subtract_level(o.side, o.price, o.quantity)?;
    ...
    fn reduce(&mut self, id: OrderId, q: Quantity, full: bool) -> Result<(), &'static str> {
        ...
        self.subtract_level(o.side, o.price, q)?;
        if q == o.quantity {
            self.orders.erase(id);
            self.live -= 1
        } else {
    ...
    fn upsert(
        ...
        if let Some(o) = self.orders.find(id).copied() {
            self.subtract_level(o.side, o.price, o.quantity)?;
            self.add_level(side, p, q)?;
            *self.orders.find_mut(id).unwrap() = Order { side, price: p, quantity: q };
            Ok(())
        } else {
