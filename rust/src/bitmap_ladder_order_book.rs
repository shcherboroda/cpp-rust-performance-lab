...
impl Default for Slot {
    fn default() -> Self {
        Self { id: 0, order: Order::make(Side::Bid, 0, 0), state: State::Empty }
    }
}
...
impl OrderBook {
    ...
    pub fn state_digest(&self) -> u64 {
        ...
        for id in ids {
            ...
            hash_u(&mut h, self.minimum + s.order.offset() as u64);
    ...
    fn add(&mut self, id: OrderId, side: Side, p: Price, q: Quantity) -> Result<(), &'static str> {
        ...
        self.slots[i] = Slot { id, order: Order::make(side, offset, q), state: State::Occupied };
        self.live += 1;
        Ok(())
    }
    ...
    fn upsert(
        ...
        if let Some(i) = self.find(id) {
            ...
            self.subtract_level(o.side(), self.minimum + o.offset() as u64, o.quantity)?;
            self.add_level(side, p, q)?;
            self.slots[i].order = Order::make(side, offset, q);
            Ok(())
        } else {
