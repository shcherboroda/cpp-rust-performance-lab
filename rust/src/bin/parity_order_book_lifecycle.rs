use low_latency_lab_benchmarks::parity_order_book::{Event, EventType, OrderBook, Side};
use std::fs::File;
use std::io::Write;
...
fn run_sample(events: &[Event]) -> u128 {
    let mut book = OrderBook::new(ORDER_COUNT);
...
fn main() {
    ...
    let layout_book = OrderBook::new(ORDER_COUNT);
    ...
    println!("benchmark: parity_order_book_lifecycle_churn_v2");
